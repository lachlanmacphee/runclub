...
export function MemberStats() {
  ...
  const fetchUserStats = useCallback(async () => {
    ...
    const resVolunteeringWaiver = (await pb
      .collection("volunteers")
      .getFullList({
        filter: pb.filter("waiver_id = {:waiverId}", { waiverId }),
      })) as Participant[];

    const resVolunteeringUser = (await pb.collection("volunteers").getFullList({
      filter: pb.filter("user_id = {:userId}", { userId }),
    })) as Participant[];

    setVolunteerCount(
      resVolunteeringUser.length + resVolunteeringWaiver.length
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-4">
        <div className="grid grid-cols-2 grid-rows-2 gap-2">
          <Card className="flex flex-col">
            <CardHeader>
              <CardTitle>5km Run</CardTitle>
              <CardDescription>
                The number of times you have run a 5km at Gunnies.
              </CardDescription>
            </CardHeader>
          ...
          <Card className="flex flex-col">
            <CardHeader>
              <CardTitle>3.5km Run</CardTitle>
              <CardDescription>
                The number of times you have run a 3.5km at Gunnies.
              </CardDescription>
            </CardHeader>
            <CardContent className="flex flex-grow justify-center items-center">
              <p className="font-bold text-5xl m-auto">
          ...
          <Card className="flex flex-col">
            <CardHeader>
              ...
              <CardDescription>
                The number of times you have volunteered at Gunnies.
              </CardDescription>
            </CardHeader>
            <CardContent className="flex flex-grow justify-center items-center">
              <p className="font-bold text-5xl m-auto">{volunteerCount}</p>
            </CardContent>
          </Card>
        </div>
        <Card>
